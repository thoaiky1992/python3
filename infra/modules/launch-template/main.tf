resource "aws_launch_template" "ecs_pgbouncer_launch_template" {
  ...
  user_data = base64encode(templatefile("${path.module}/pgbouncer-user-data.sh.tftpl", { ECS_CLUSTER_NAME = "${var.ecs_cluster.name}" }))
}
...
resource "aws_launch_template" "ecs_db_launch_template" {
  ...
  user_data = base64encode(templatefile("${path.module}/db-user-data.sh.tftpl", { ECS_CLUSTER_NAME = var.ecs_cluster.name }))
}
