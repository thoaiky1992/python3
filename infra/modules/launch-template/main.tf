...
resource "aws_launch_template" "ecs_api_launch_template" {
  ...
  network_interfaces {
    ...
    security_groups             = [var.security_groups["api-security-group"].id] # Replace with your security group ID
  }
...
resource "aws_launch_template" "ecs_redis_launch_template" {
  ...
  network_interfaces {
    associate_public_ip_address = false                                               # Ensure this is false for private subnet
    subnet_id                   = var.private_subnet_ids[2]                           # Replace with your private subnet ID
